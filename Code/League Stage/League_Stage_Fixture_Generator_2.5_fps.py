...
def select_opponent(selected, excluded):
    ...
    precision = Decimal('0.01')  # Use Decimal for maintaining precision
    ...
    for team in available_teams:
        ...
        if selected_city in coordinates and team_city in cities:
            ...
            distance = max(Decimal(coordinates[selected_city][away_idx]), precision)
            inverse_distance = Decimal('1') / distance
            weights.append(inverse_distance)
            team_data.append((team, inverse_distance))

    total_weight = sum(weights)
    normalized_weights = [weight / total_weight for weight in weights]

    random_num = Decimal(random.uniform(0, 1))
    current_sum = Decimal('0')

    for (team, _), weight in zip(team_data, normalized_weights):
        current_sum += weight
