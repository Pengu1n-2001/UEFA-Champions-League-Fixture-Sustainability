...
def calculate_team_stats(input_file, output_file):
    ...
    csv_data = [['team'] + [f'game_{i}' for i in range(1, 9)] + ['average_distance_travelled_per_away_game',
                                                                  'total_distance_travelled']]

    for team in team_stats:
        total_distance = sum(team_stats[team][f'game_{i}'] for i in range(1, 9))
        games = [team_stats[team][f'game_{i}'] for i in range(1, 9)]
        ...
        if any(map(lambda x: x > 0, games[-2:])):
            ...
            average_distance_travelled = round(total_distance / 3) if team_stats[team]['games_travelled_to'] > 0 else 0
...
input_file_path = '../../Fixtures/league_stage_and_poko_fixtures_distances.csv'
output_file_path = '../../Stats/team_stats.csv'
